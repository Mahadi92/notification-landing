...
const Web3State = (props) => {
    ...
    useEffect(() => {
        ...
        console.log("previouslyConnectedWallet", previouslyConnectedWallets);
        if (previouslyConnectedWallets?.length) {
            async function setWalletFromLocalStorage() {
                const walletConnected = await connect({autoSelect: { label: previouslyConnectedWallets[0], disableModals: true }});
            }
    ...
    const connect = async (connectParams) => {
        console.log("hello", connectParams);
        setLoading();
        ...
        let accounts;
        try {
			if ("autoSelect" in connectParams)
            accounts = await onboard.connectWallet(connectParams);
			else accounts = await onboard.connectWallet();
